...
fn given_world(world: &mut CylindersWorld, _: &[String]) {
    world.world = rtxch_lib::World::new();
}

#[when(regex = r"w ← default_world()")]
#[given(regex = r"w ← default_world()")]
fn given_default_world(world: &mut CylindersWorld, _: &[String]) {
    world.world = rtxch_lib::World::default_world();
}
#[given(regex = r"(shape) ← plane\(\).+$")]
fn given_plane(world: &mut CylindersWorld, matches: &[String]) {
    let plane = Plane::new();
...
fn given_lower_plane(world: &mut CylindersWorld, matches: &[String]) {
    let plane = Plane::new();
    plane.borrow_mut().set_transform(&Matrix::translate(0.0,-1.0,0.0));
...
fn given_upper_plane(world: &mut CylindersWorld, matches: &[String]) {
    let plane = Plane::new();
    plane.borrow_mut().set_transform(&Matrix::translate(0.0,1.0,0.0));
    plane.borrow_mut().get_mut_material().reflective = 1.0;
    world.shape.insert(matches[0].to_string(), plane);
...
#[given(regex = r"(.+) ← (cylinder|local_intersect|local_normal_at|cube|reflected_color|refracted_color|intersections|point|vector|ray|intersect|translation|scaling|normal_at|rotation_z|color|point_light|sphere|prepare_computations)\((.*)\)$")]
#[when(regex = r"(.+) ← (cylinder|local_intersect|local_normal_at|cube|reflected_color|refracted_color|intersections|point|vector|ray|sphere|intersect|translation|scaling|normal_at|point_light|intersect_world|prepare_computations|shade_hit|color_at)\((.*)\)")]
fn given_item(world: &mut CylindersWorld, matches: &[String]) {
    create_item(world, matches);
}

#[given(regex = r"^(floor|ball) ← (plane|sphere)\(\) with:$")]
fn floor_plane(world: &mut CylindersWorld, step: &Step, matches: &[String]) {
    let name = &matches[0];
...
fn shape_has(world: &mut CylindersWorld, step: &Step, matches: &[String]) {
...
fn sphere1(world: &mut CylindersWorld) {
...
fn sphere2(world: &mut CylindersWorld) {
    let sphere = Sphere::new();
...
fn sphere2_alter(world: &mut CylindersWorld) {
    let sphere = Sphere::new();
...
fn check_result(world: &mut CylindersWorld, matches: &[String]) {
...
fn create_item(world: &mut CylindersWorld, matches: &[String]) {
...
fn add_sphere(world: &mut CylindersWorld, matches: &[String]) {
...
fn first(world: &mut CylindersWorld, matches: &[String]) {
...
fn set_light(world: &mut CylindersWorld, _: &[String]) {
...
fn intershape(world: &mut CylindersWorld, matches: &[String]) {
...
fn then_light(world: &mut CylindersWorld, _: &[String]) {
    let light = world.plight.get(&"light".to_string()).unwrap();
...
fn check_inf(world: &mut CylindersWorld, _: &[String]) {
...
fn contains(world: &mut CylindersWorld, matches: &[String]) {
    let sphere = world.shape.get(&matches[0]).unwrap();
...
fn check_shadow(world: &mut CylindersWorld, matches: &[String]) {
...
fn check_inner_color(world: &mut CylindersWorld, _: &[String]) {
...
fn check_prop_comps(world: &mut CylindersWorld, matches: &[String]) {
...
fn no_light(world: &mut CylindersWorld, _: &[String]) {
    assert!(world.world.get_point_lights().len() == 0);
}

#[then(regex = r"xs\.(count) = (.+)")]
fn check_prop_intersection(world: &mut CylindersWorld, matches: &[String]) {
...
#[then(regex = r"xs\[(\d)\]\.(t) = (.+)")]
fn check_prop_intersection_idx(world: &mut CylindersWorld, matches: &[String]) {
    let xs = world.inter_list.get(&"xs".to_string()).unwrap();
...
fn main() {
    futures::executor::block_on(CylindersWorld::run(
        "tests/features/cylinders.feature",
    ));
}